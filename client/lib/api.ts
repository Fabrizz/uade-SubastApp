...
export const API_BASE =
  process.env.EXPO_PUBLIC_API_URL ?? "https://cly-subastapp.fabriziob.com";
...
function truncateTrace(body: unknown): string {
  ...
  const text = (typeof body === "string" ? body : JSON.stringify(body)).replace(
    /\s*\n\s*/g,
    "",
  );
  return text.length > TRACE_MAX_LEN
    ? `${text.slice(0, TRACE_MAX_LEN)}…`
    : text;
}
...
api.use({
  ...
  async onResponse({ request, response }) {
    ...
    console.log(
      `[RESPONSE] [${response.status}] ${request.method} ${request.url} ${truncateTrace(body)}`,
    );
    ...
    if (!response.ok) {
      ...
      console.log(
        `[error] [api] ${label} — ${request.method} ${request.url}`,
        truncateTrace(body),
      );
    }
    return response;
  ...
  onError({ request, error }) {
    console.log(
      `[error] [api] network error — ${request.method} ${request.url}`,
      truncateTrace(error instanceof Error ? error.message : error),
    );
  },
});
